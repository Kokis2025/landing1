...
const BookCarousel: React.FC<{ books: Book[] }> = ({ books }) => {
    ...
    // A simple heuristic to show arrows. A more robust solution might use ResizeObserver
    // to check if scrollWidth > clientWidth. For now, we show if there are enough books.
    const showArrows = true; // For simplicity, we'll let CSS handle hiding on very small screens if needed.
    ...
    return (
        <div className="relative group">
            <div
                ...
                style={{ scrollbarWidth: 'none' }} /* Oculta la barra de desplazamiento en Firefox */
            >
                {books.map((book) => (
                    <div key={book.id} className="snap-center flex-shrink-0 w-4/5 sm:w-1/2 md:w-1/3 lg:w-1/4 p-2">
                        <BookCard {...book} />
                    </div>
                ))}
            </div>
            {showArrows && (
                <>
                    <button onClick={() => scroll('left')} className="absolute top-1/2 left-0 z-10 -translate-y-1/2 bg-white/80 hover:bg-white rounded-full p-2 shadow-lg transition-all opacity-0 group-hover:opacity-100 duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[var(--color-primary)]" aria-label="Anterior">
                    ...
                    <button onClick={() => scroll('right')} className="absolute top-1/2 right-0 z-10 -translate-y-1/2 bg-white/80 hover:bg-white rounded-full p-2 shadow-lg transition-all opacity-0 group-hover:opacity-100 duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[var(--color-primary)]" aria-label="Siguiente">
